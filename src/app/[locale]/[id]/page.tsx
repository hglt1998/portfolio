...
function ProjectDetailPage() {
	...
	return (
		<div className="container flex flex-col mx-auto p-6 max-w-4xl bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 my-10 rounded-lg shadow-md">
			{project.properties.active.checkbox ? (
				<span className="self-end bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">{t("active")}</span>
			) : (
				<span className="self-end bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">{t("unactive")}</span>
			)}
			...
			{images.length > 0 && (
				<section className="mb-8">
					<h2 className="text-xl font-semibold text-indigo-500 dark:text-indigo-300 mb-4">{locale === "es" ? "Galería de Imágenes" : "Image Gallery"}</h2>
			...
			<div className="text-center">
				<Link
					rel="_noopener"
					target="_blank"
					className="flex text-center justify-center gap-3 items-center bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-colors md:w-3/12 md:m-auto duration-200"
