...
const TechnologiesBlock = () => {
	...
	return (
		<div className="lg:max-w-4xl sm:max-w-full mx-auto py-8 px-4 bg-indigo-100 dark:bg-gray-800 rounded-lg shadow-lg text-center">
			<h2 className="text-3xl font-bold text-gray-800 dark:text-gray-200 mb-6">{t("technologiesTitle")}</h2>
			<p className="text-gray-700 dark:text-gray-300 mb-6">{t("technologiesIntro")}</p>
			...
			<div className="grid grid-cols-1 w-full sm:grid-cols-4 gap-4">
				{technologies.map((tech) => (
					<div key={tech.name} className="text-left mb-4">
						...
						<span className="text-sm text-gray-600 dark:text-gray-400 ml-1">{tech.level}%</span>
					</div>
				))}
			</div>
		</div>
